...
async function checkItemsTables() {
  await client.connect();
  
  console.log('🔍 Buscando tablas relacionadas con items...\n');
  
  // Buscar todas las tablas que contengan "item" en el nombre
  ...
  // Ver estructura de la tabla principal de items
  if (tables.rows.length > 0) {
    const mainTable = tables.rows.find(r => r.table_name === 'pokemon_v2_item') || tables.rows[0];
...
checkItemsTables().catch(console.error);
